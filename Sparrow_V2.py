...
class Sparrow():
    ...
    def _draw_arrow(self, startpoint:tuple, velocity:tuple, color:tuple, canvas):
        '''Draw arrow from startpoint to (startpoint+velocity)
            startpoint: a tuple of (2,)
            velocity: a tuple of (2,)
            color: a tuple of (3,) between 0~255
            canvas: the canvas of pygame'''
        grid_x, grid_y = startpoint

        # 计算矢量的终点
        scale = 10
        end_x = grid_x + int(velocity[0] * scale)
        end_y = grid_y + int(velocity[1] * scale)

        # 绘制合力的矢量 (紫色)
        pygame.draw.aaline(canvas, color, (grid_x, grid_y), (end_x, end_y))

        # 绘制箭头（位于矢量的末端）
        arrow_length = 10  # 箭头长度
        arrow_angle = math.atan2(velocity[1], velocity[0])  # 计算矢量方向的角度

        # 箭头两侧的偏移角度
        arrow_angle_left = arrow_angle + math.pi / 6  # 左侧箭头角度
        arrow_angle_right = arrow_angle - math.pi / 6  # 右侧箭头角度

        # 箭头的两个点
        arrow_end_left = (end_x - int(arrow_length * math.cos(arrow_angle_left)),
                          end_y - int(arrow_length * math.sin(arrow_angle_left)))

        arrow_end_right = (end_x - int(arrow_length * math.cos(arrow_angle_right)),
                           end_y - int(arrow_length * math.sin(arrow_angle_right)))
    ...
    def _render_frame(self):
        ...
        if self.draw_auxiliary:
            ...
            # 障碍物速度向量:
            dynamic_obs_center = self.vec_dynamic_obs_P.float().mean(dim=-2)[self.ri].cpu().numpy() # (O,2)
            dynamic_obs_velocity = self.Obs_V_tensor[self.ri, :, 0, :].cpu().numpy() # (O,2)
            for o in range(self.O):
                self._draw_arrow(dynamic_obs_center[o], dynamic_obs_velocity[o], color=(0,0,0), canvas=self.canvas)

        car_center = self._world_2_grid(self.car_state[self.ri, 0:2]).cpu().numpy()  # (2,)
